...
def main_loop_function(args):
    ...
    if (check_file_exists(f'/mnt/j/isee_remote_data/himawari_AshRGB_only_red_modified_recipe_gamma_3/{yyyy}{mm}/{yyyy}{mm}{dd}{hh}{mn}.png') == True):
        return
    ...
    data_red    = AshRGB_data(data_diff_tbb_13_15,  -3.0E0,   7.5E0,  3.0E0, 0)
    ...
    try:
        os.makedirs(f'/mnt/j/isee_remote_data/himawari_AshRGB_only_red_modified_recipe_gamma_3/{yyyy}{mm}')
    except FileExistsError:
        pass
    
    #画像の保存 (保存先は要指定)
    fig.savefig(f'/mnt/j/isee_remote_data/himawari_AshRGB_only_red_modified_recipe_gamma_3/{yyyy}{mm}/{yyyy}{mm}{dd}{hh}{mn}.png')
    now = str(datetime.datetime.now())
...
if (__name__ == '__main__'):
    # プロセス数
    num_processes = 2   #1推奨(要するに並列処理不可: ftp最大接続数10の制限の為)
