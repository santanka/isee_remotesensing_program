    ...
    for count_i in range(7):
        ...
        if data.all() != 0:
            try:
                ...
                data_filled = np.zeros((line_number, pixel_number))
...
images = []
for filename in sorted(os.listdir(dir_figure), reverse=True):
    if filename.endswith('.png'):
        filepath = os.path.join(dir_figure, filename)
        image = Image.open(filepath)
        images.append(image)
gif_filepath = os.path.join(dir_figure, 'back_trajectory.gif')
