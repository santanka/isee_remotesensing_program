...
dir_figure = f'/mnt/j/isee_remote_data/JST/chla_oceanic_current_plot_widespace'
...
def read_central_point(year, month):
    ...
    central_point_csv_path = f'chla_central_point_JST_{yyyy}{mm}_2.csv'
    #1行目をスキップして読み込む
...
def get_data_daily(year_int, month_int, day_int):
    ...
    for hour_idx in range(8):
        #12時から翌9時までのデータを使用
        date_JST_start = datetime.datetime(year_int, month_int, day_int, 12, 0, 0)
        date_JST = date_JST_start + datetime.timedelta(hours=hour_idx*3)
        date_UTC = date_JST - datetime.timedelta(hours=9)
        lat_data, lon_data, water_u_data, water_v_data = get_data(date_UTC.year, date_UTC.month, date_UTC.day, date_UTC.hour)
