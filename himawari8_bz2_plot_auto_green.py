...
def main_loop_function(args):
    ...
    if (check_file_exists(f'/mnt/j/isee_remote_data/himawari_AshRGB_only_green/{yyyy}{mm}/{yyyy}{mm}{dd}{hh}{mn}.png') == True):
        return
    ...
    if (data_tbb_11.all() == 0 or data_tbb_14.all() == 0):
        return
    ...
    data_red    = data_green * 0E0  #all zero
    data_blue   = data_green * 0E0  #all zero
    ...
    try:
        os.makedirs(f'/mnt/j/isee_remote_data/himawari_AshRGB_only_green/{yyyy}{mm}')
    except FileExistsError:
        pass
    
    #画像の保存 (保存先は要指定)
    fig.savefig(f'/mnt/j/isee_remote_data/himawari_AshRGB_only_green/{yyyy}{mm}/{yyyy}{mm}{dd}{hh}{mn}.png')
    now = str(datetime.datetime.now())
