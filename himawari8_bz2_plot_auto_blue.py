...
def main_loop_function(args):
    ...
    if (check_file_exists(f'/mnt/j/isee_remote_data/himawari_AshRGB_only_blue/{yyyy}{mm}/{yyyy}{mm}{dd}{hh}{mn}.png') == True):
        return
    ...
    data_red    = data_blue * 0E0  #all zero
    data_green  = data_blue * 0E0  #all zero
    ...
    try:
        os.makedirs(f'/mnt/j/isee_remote_data/himawari_AshRGB_only_blue/{yyyy}{mm}')
    except FileExistsError:
        pass
    
    #画像の保存 (保存先は要指定)
    fig.savefig(f'/mnt/j/isee_remote_data/himawari_AshRGB_only_blue/{yyyy}{mm}/{yyyy}{mm}{dd}{hh}{mn}.png')
    now = str(datetime.datetime.now())
