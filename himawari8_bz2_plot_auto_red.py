...
def main_loop_function(args):
    ...
    if (check_file_exists(f'/mnt/j/isee_remote_data/himawari_AshRGB_only_red/{yyyy}{mm}/{yyyy}{mm}{dd}{hh}{mn}.png') == True):
        return
    ...
    try:
        os.makedirs(f'/mnt/j/isee_remote_data/himawari_AshRGB_only_red/{yyyy}{mm}')
    except FileExistsError:
        pass
    
    #画像の保存 (保存先は要指定)
    fig.savefig(f'/mnt/j/isee_remote_data/himawari_AshRGB_only_red/{yyyy}{mm}/{yyyy}{mm}{dd}{hh}{mn}.png')
    now = str(datetime.datetime.now())
